...
public class QnaDBBean {
	...
	public List<QnaDataBean> getArticles(int count, int book_id) {
		...
		try {
			...
			pstmt = conn
					.prepareStatement("select *from qna where book_id=" + book_id + "order by group_id desc, qora asc");
			rs = pstmt.executeQuery();
	...
	public int updateArticle(QnaDataBean article) {
		...
		try {
			...
			pstmt = conn.prepareStatement("update qna set qna_content=? where qna_id=?");
