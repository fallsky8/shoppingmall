	...
	<form id="uForm" method="post" action="/shoppingmall/modifyForm.do">
		<ul>
			<li><label for="passwd">비밀번호</label> <input id="passwd"
				...
				placeholder="6~16자의 문자 또는 숫자" maxlength="16"> <input id="id"
				name="id" type="hidden" value="${sessionScope.id }"> <input
				type="submit" id="modify" value="정보수정">
		</ul>
	</form>
	...
	<form id="dForm" method="post" action="/shoppingmall/deletePro.do">
		<ul>
			<li><label for="passwd">비밀번호</label> <input id="passwd"
				...
				placeholder="6~16자의 문자 또는 숫자" maxlength="16"> <input id="id"
				name="id" type="hidden" value="${sessionScope.id }"> <input
				type="submit" id="delete" value="탈퇴"> <small class="cau">
					[탈퇴 버튼을 누르면 바로 탈퇴됨] </small>
		</ul>
	</form>
	<button id="shopMain"
