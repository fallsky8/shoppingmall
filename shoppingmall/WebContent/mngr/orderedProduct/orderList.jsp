	...
	<c:if test="${count>0 }">
		<table>
			...
			<c:forEach var="i" begin="0" end="${buyLists.size()-1 }">
				<c:set var="buyList" value="${buyLists.get(i) }"></c:set>
				<c:set var="pid" value="${buyList.getBuy_id() }"></c:set>
				<c:if test="${i+1 >buyLists.size()-1}">
					<c:set var="nid" value="0"></c:set>
				</c:if>
				<c:if test="${i+1<=buyLists.size()-1 }">
					<c:set var="nid" value="${buyLists.get(i+1).getBuy_id() }"></c:set>
				</c:if>
				<tr>
					...
					<td><fmt:formatNumber value="${buyList.getBuy_price() }"
							type="number" pattern="#,##0"></fmt:formatNumber>원</td>
					<td>${buyList.getBuy_count() }</td>
					<td><c:set var="amount"
							value="${buyList.getBuy_count()*buyList.getBuy_price() }"></c:set>
						<c:set var="total" value="${total+amount }"></c:set> <fmt:formatNumber
							value="${amount }" type="number" pattern="#,##0"></fmt:formatNumber>원</td>
				...
				<c:if test="${pid!=nid }">
					<tr>
						<td colspan="12" class="b">주문금액: <fmt:formatNumber
								value="${total }" type="number" pattern="#,##0"></fmt:formatNumber>원
						</td>
					</tr>
					<c:set var="total" value="0"></c:set>
					<c:set var="pid" value="${nid }"></c:set>
