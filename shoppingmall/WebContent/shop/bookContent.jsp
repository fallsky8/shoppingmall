...
<div id="showBook">
	<table class="vhcenter">
		...
		<tr>
			<td width="500">저자 : ${book.getAuthor() }</td>
		</tr>
		<tr>
		...
		<tr>
			<td width="500"><c:set var="price"
				...
				<strong class="bred">판매가 :<c:set var="rPrice"
						value="${price*((100.0-rate)/100) }"></c:set> <fmt:formatNumber
						value="${rPrice }" type="number" pattern="#,##0"></fmt:formatNumber>
					원
			</strong>
		<tr>
...
<div id="showQna">
	...
	<c:if test="${count>0 }">
		<c:forEach var="qna" items="${qnaLists }">
			<ul>
				<li><c:set var="writer" value="${qna.getQna_writer() }"></c:set>
				...
				<li><c:if test="${sessionScope.id==writer }">
						...
						<button id="delete"
							name="${qna.getQna_id() },${book_id},${book_kind}"
