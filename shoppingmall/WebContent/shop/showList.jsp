...
<div id="shop" class="box2">
	<c:if test="${book_kind=='100' }">
		<c:set var="book_kindName" value="문학"></c:set>
	</c:if>
	<c:if test="${book_kind=='200' }">
		<c:set var="book_kindName" value="외국어"></c:set>
	</c:if>
	<c:if test="${book_kind=='300' }">
		<c:set var="book_kindName" value="컴퓨터"></c:set>
	</c:if>

	<c:if test="${book_kind=='all' }">
		<c:set var="book_kindName" value="전체"></c:set>
		<c:set var="display" value="전체 목록"></c:set>
	</c:if>
	<c:if test="${book_kind!='all' }">
		<c:set var="display" value="[${book_kindName }] 분류의 목록"></c:set>
	</c:if>
		...
		<table class="vhcenter">
			...
			<tr height="30">
				<td width="350"><c:set var="price"
						...
						value="${price }" type="number" pattern="#,##0"></fmt:formatNumber>원<br>
					<strong class="bred">판매가:<fmt:formatNumber
